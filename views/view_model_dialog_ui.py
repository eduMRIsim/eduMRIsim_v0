...
class ModelViewLabel(ImageLabel):
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        if self.pixmap_item.isUnderMouse():
            coordinate_in_pixmap = self.pixmap_item.mapFromScene(
                self.mapToScene(event.pos())
            )
            # convert to model coordinates
            self.mouse_moved_signal.emit(
                int(abs(coordinate_in_pixmap.y())),
                int(abs(coordinate_in_pixmap.x())),
                self.current_slice,
            )
        else:
            self.mouse_moved_signal.emit(
                -1, -1, -1
            )  # signal that mouse is not over the pixmap
    ...
    def update_text_item(self):
        ...
        x = (
            pixmap_rect.right() - text_rect.width() - 15
    ...
    def wheelEvent(self, event):
        ...
        if self.pixmap_item.isUnderMouse():
            ...
            coordinate_in_pixmap = self.pixmap_item.mapFromScene(
                self.mapToScene(mouse_position)
            )
            # convert to model coordinates
            self.mouse_moved_signal.emit(
                int(abs(coordinate_in_pixmap.y())),
...
class ViewModelDialog(QDialog):
    ...
    def createInformationLabels(self):
        ...
        self.tissue_property_value_display.setStyleSheet(
            "QLineEdit { border: 1px solid  #BFBFBF; }"
        )
        ...
        self.information_label_layout.addWidget(
            self.tissue_property_value_display, 1, 0
        )
