...
class Scanner:
    ...
    def __init__(self):
        ...
        self.MRI_data_synthesiser = MRIDataSynthesiser()  # Create an instance of the MRIDataSynthesiser class. The MRIDataSynthesiser class is responsible for synthesising MRI data from the model and scan parameters. Not relevant for SEP project.
    ...
    def _get_image_data_from_signal_array(self, image_geometry: ImageGeometry, model: Model,
                                          signal_array: np.ndarray) -> np.ndarray:
        ...
        x_sample = np.linspace(-(model_extentX / 2) + (model.XDimRes_mm / 2),
                               (model_extentX / 2) - (model.XDimRes_mm / 2), int(model.XDim))
        y_sample = np.linspace(-(model_extentY / 2) + (model.YDimRes_mm / 2),
                               (model_extentY / 2) - (model.YDimRes_mm / 2), int(model.YDim))
        z_sample = np.linspace(-(model_extentZ / 2) + (model.ZDimRes_mm / 2),
                               (model_extentZ / 2) - (model.ZDimRes_mm / 2), int(model.ZDim))

        inter_func = interpolate.RegularGridInterpolator((y_sample, x_sample, z_sample), signal_array, method='linear',
                                                         bounds_error=False,
                                                         fill_value=0)  # Create an interpolation function that can be used to sample the signal array at any point in the FOV. The fill_value is set to 0, which means that if the point is outside the FOV, the signal intensity will be 0. The x and y coordinates are swapped in the interpolation function because the x-coordinate corresponds to the columns of the signal array and the y-coordinate corresponds to the rows.
        ...
        x_FOV = np.linspace(- image_geometry.extentX_mm / 2 + dFOVx / 2, image_geometry.extentX_mm / 2 - dFOVx / 2,
                            NFOVx)
        y_FOV = np.linspace(- image_geometry.extentY_mm / 2 + dFOVy / 2, image_geometry.extentY_mm / 2 - dFOVy / 2,
                            NFOVy)
        z_FOV = np.linspace(- 1 / 2 + dFOVz / 2, 1 / 2 - dFOVz / 2,
                            NFOVz)  # This is temporary until slice selection is implemented. Pretend slice thickness is 1 mm

        X_FOV, Y_FOV, Z_FOV = np.meshgrid(x_FOV, y_FOV, z_FOV)
        # Convert the FOV coordinates to LPS coordinates
        X_FOV_LPS = X_FOV * image_geometry.axisX_LPS[0] + Y_FOV * image_geometry.axisY_LPS[0] + Z_FOV * \
                    image_geometry.axisZ_LPS[0] + image_geometry.origin_LPS[0]
        Y_FOV_LPS = X_FOV * image_geometry.axisX_LPS[1] + Y_FOV * image_geometry.axisY_LPS[1] + Z_FOV * \
                    image_geometry.axisZ_LPS[1] + image_geometry.origin_LPS[1]
        Z_FOV_LPS = X_FOV * image_geometry.axisX_LPS[2] + Y_FOV * image_geometry.axisY_LPS[2] + Z_FOV * \
                    image_geometry.axisZ_LPS[2] + image_geometry.origin_LPS[2]
        image_data = inter_func((Y_FOV_LPS, X_FOV_LPS, Z_FOV_LPS))  # Sample the signal array at the points in the FOV
        return image_data[:, :,
               0]  # image_data is a 3D array. The third dimension is 1 because the z dimension is 1. We remove the third dimension to get a 2D array.
